<!-- FOOTER -->
<footer class="grid">
    <!-- Footer - top -->
    <!-- Image-->
<!--    <div class="s-12 l-3 m-row-3 margin-bottom background-image" style="background-image:url(/assets/img/img-04.jpg)"></div>-->

    <div class="s-12 m-8 l-4 padding-2x margin-bottom background-dark">
        <h2 class="text-strong text-uppercase">Who We Are?</h2>
        <p>
            We assist in attracting and admitting international students.<br>
            We support cooperation and exchange between domestic and international universities.<br>
            We assist in the establishment of Korean language centers in high schools and universities abroad.<br>
            We support universities in promoting trade in various fields.<br>
        </p>
    </div>

    <div class="s-12 m-8 l-4 padding-2x margin-bottom background-dark">
        <h2 class="text-strong text-uppercase">Where We Are?</h2>
        <p>SEOUL, South Korea</p>
        <img class="full-img" src="/assets/img/map.png" alt=""/>
    </div>

    <div class="s-12 m-8 l-4 padding-2x margin-bottom background-dark">
        <h2 class="text-strong text-uppercase">Contact Us</h2>
        <p><b class="text-primary margin-right-10">P</b> +82 10 9974 4641</p>
        <p><b class="text-primary margin-right-10">M</b> <a class="text-primary-hover" href="mailto:contact@sampledomain.com">powerremit@naver.com</a></p>
        <p><b class="text-primary margin-right-10">M</b> <a class="text-primary-hover" href="mailto:office@sampledomain.com">henung2001@naver.com</a></p>
    </div>

    <!-- Footer - bottom -->
<!--    <div class="s-12 text-center margin-bottom">-->
<!--        <p class="text-size-12">Copyright 2019, Vision Design - graphic zoo</p>-->
<!--        <p class="text-size-12">All images have been purchased from Bigstock. Do not use the images in your website.</p>-->
<!--        <p><a class="text-size-12 text-primary-hover" href="http://www.myresponsee.com" title="Responsee - lightweight responsive framework">Design and coding by Responsee Team</a></p>-->
<!--    </div>-->
</footer>